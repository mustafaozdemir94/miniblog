    ...
    return Scaffold(
        appBar: AppBar(
          ...
          actions: [
            IconButton(
                onPressed: () async {
                  bool? result = await Navigator.of(context).push(MaterialPageRoute(builder: (ctx) => const AddBlog()));
                  if (result == true) {
                    fetchBlogs();
                  }
                },
                  ...
                  itemBuilder: (ctx, index) => InkWell(
                    onTap: () {
                      Navigator.of(context)
                          .push(MaterialPageRoute(builder: (ctx) => BlogDetails(blogId: blogList[index].id!)));
                    },
                    child: BlogItem(
                      blog: blogList[index],
                    ),
